...
namespace osu.Framework.Graphics.UserInterface.Graph
{
    ...
    public partial class Anchor : IGraphAnchor
    {
        ...
        public Anchor(Graph parent, Vector2 pos)
            : this(parent, pos, null)
        {
        }

        public Anchor(Graph parent, Vector2 pos, IGraphInterpolator interpolator)
            : this(parent, pos, interpolator, null)
        {
        }

        [JsonConstructor]
        public Anchor(Graph parent, Vector2 pos, IGraphInterpolator interpolator, TensionAnchor tensionAnchor)
            : base(parent)
        {
            InitializeComponent();
            SetCursor();
        ...
        private void UpdateInterpolatorStuff()
        {
            ...
            foreach (var item in items)
            {
                ...
                {
                    mi.Icon = interpolator == type ? new PackIcon { Kind = PackIconKind.RadioboxMarked } : new PackIcon { Kind = PackIconKind.RadioboxBlank };
                }
            }
